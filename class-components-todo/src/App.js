...
class App extends Component {
  ...
  delete = (todo) => {
    this.setState({
      all: this.state.all.filter(
        (item) => this.state.all.indexOf(item) !== this.state.all.indexOf(todo)
      ),
    });
  };
  ...
  editTodo = (id) => {
    this.setState({
      all: this.state.all.map((item) =>
        item.id === id
          ? { ...item, action: this.state.editToDo, isEdit: false }
          : item
      ),
    });
  };
  ...
  checkAll = () => {
    if (this.count("completed") < this.count()) {
      this.setState({
        all: this.state.all.map((item) => (item = { ...item, done: true })),
      });
    } else {
      this.setState({
        all: this.state.all.map((item) => (item = { ...item, done: false })),
      });
    }
  };
  ...
  clear = () => {
    this.setState({
      all: this.state.all.filter((item) => item.done === false),
    });
  };
